...
export class FreehandSmoother {
  private readonly defaultOptions: Required<SmootherOptions> = {
    ...
    velocityWeight: 0.2,
    curvatureWeight: 0.3,
    minDistance: 0.2, // lower minimum distance threshold
    maxPoints: 8,
    pressureSensitivity: 0.5,
    tiltSensitivity: 0.3,
    velocityThreshold: 800,
    samplingRate: 5, // lower sampling interval
  };
  ...
  process(
    ...
    // Return the first point immediately
    ...
    // Sampling rate control — skip points that arrive too close together
    ...
    // Distance check with dynamic minimum distance adjustment
    const distanceOk = this.checkDistance(point);
    if (!distanceOk && this.points.length > 1) {
      // If points are too close but enough time has passed, still process it
      const timeDiff = timestamp - this.lastProcessedTime;
      if (timeDiff < 32) { // 32ms ≈ 30fps
        return null;
      }
    }
  ...
  private checkDistance(point: Point): boolean {
    ...
    // Dynamic minimum distance based on current velocity
  ...
  private calculateDynamicParameters(strokePoint: StrokePoint) {
    ...
    // Pressure adaptation — gentle response
    ...
    // Velocity adaptation — smooth transition
    ...
    // Tilt adaptation — gentle response
